...
- corpus/corpus_match.py — `corpus-match` (MEDIUM/MEDIUM) — TF-IDF cosine (Jaccard fallback) vs **98 vetted entries (one per unique incident — de-duplicated) totalling ~$7.1B in losses (2020-2026, 13 chains)**

Corpus is regenerated by `python packages/detectors/tryanneal_detectors/corpus/build_corpus.py` from raw research dumps in `corpus/research/`. The build script tolerates malformed JSON (truncated files, embedded control characters) via greedy object-level recovery. Manual handwritten entries in `patterns.manual.json` are merged in unless a research entry with the same id overrides them. The build then runs an incident-level de-dup pass (`INCIDENT_ALIASES` + a defensive protocol/year/month collapse) so the same real-world exploit is never counted twice — keeping the richer record per incident.
