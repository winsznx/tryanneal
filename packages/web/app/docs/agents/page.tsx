  ...
  return (
    <article>
      ...
      <P>
        ...
        corroboration rule that needs &ge;2 independent sources per finding, and memoization by code hash). Any
        agent can read that verdict back — no re-audit needed. TryAnneal is itself a registered ERC-8004 agent
        (mainnet agent <strong>#131</strong>).
      </P>
...
tool  → {
  ...
  "recommendation": "No critical/high findings on record — safe to compose."
      ...
      <UL>
        ...
        <LI><strong>REST (one HTTP call):</strong> <Code>GET https://tryanneal.xyz/api/safety/&#123;addressOrHash&#125;?network=mantle</Code> — see <A href="/docs/safety-oracle">Safety Oracle API</A>.</LI>
        <LI><strong>CLI:</strong> <Code>npx anneal audit ./Contract.sol --network mantle</Code> — see <A href="/docs/cli">CLI</A>.</LI>
        <LI><strong>Humans:</strong> paste a contract at <A href="/try">tryanneal.xyz/try</A> — plain-English verdict, no setup.</LI>
      </UL>
